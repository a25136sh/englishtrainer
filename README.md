...
APIサーバーに接続して使用するにはdotenvの設定が必要です。.envファイルをトップディレクトリに作成してURLを設定してください。

```sh
echo "VITE_API_HOST=http://APIサーバーのホスト名" > .env
```

APIサーバーをローカルで立ち上げる方法はgenred_englishリポジトリの手順を参照してください。その際にCORSの設定が必要となるので注意してください。
